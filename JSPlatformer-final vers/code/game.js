...
DOMDisplay.prototype.drawActors = function() {
  ...
  this.level.actors.forEach(function(actor) {
    var rect;
    if (playerStatus == "powerUp" && actor.type == "player")
    {
      rect = wrap.appendChild(elt("div",
      "actor player2 "));
    }
    else {
      rect = wrap.appendChild(elt("div",
...
var wobbleSpeed2 = 3, wobbleDist2 = 0.09;

Fruit.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed2;
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    ...
    this.actors = this.actors.filter(function(other) {   //removes coin that the player touched
      return other != actor;
    });
    ...
    if (!this.actors.some(function(actor) {
      ...
      console.log("add something here, yeah");
    }
  }
  else if (type == "fruit") {
    ...
    playerStatus = "powerUp";

  }
  else if (type == "enemy" && this.status == null) { //this.status==null means if there's no status assigned to it already. if you don't put this here, then you'll create a loop where the game will keep losing(assigning lost status) over and over again
    if (playerStatus == "powerUp")
    {
      this.actors = this.actors.filter(function(other) {
        return other != actor;
      });
    ...
    {
      this.status = "lost";     //sets status of game to "lost"
      this.finishDelay = 1;     //delays the lost status for 1 second to account for the death animation
    }
